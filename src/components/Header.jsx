...
const Header = () => {
  ...
  return (
    <>
      <nav className="md:px-16 bg-gray-100  dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <a className="flex items-center space-x-3 rtl:space-x-reverse">
            <span className=" text-sm self-center md:text-2xl font-semibold whitespace-nowrap dark:text-white">Nilanjan Chakraborty</span>
          </a>
          <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            <button type="button" className="text-white bg-orange-500 shadow rounded-full px-4 py-2">{actionButton.title}</button>
          ...
          <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" style={{display: mobileMenuOpen ? 'block' : '' }}>
            ...
            {menuLinks.map((link) => (
              <li>
                <a key={link.id} onClick={() => { activeMenu(link.id)}} href={link.link} className={`block py-2 px-3 ${link.isActive ? 'bg-orange-600 text-white' : 'text-gray-600'} md:bg-transparent md:text-gray-600 ${link.isActive && 'border-b-2 border-orange-500'} md:p-0 md:dark:text-orange-600 md:hover:text-orange-600`}  >
                {link.title}
                </a>
              </li>
              ))}
